...
This document lists common issues encountered during SciTops smoke testing and how to resolve them.
...
3. Or stop SciTops services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p scitops-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it scitops-gateway sh
```

#### Clean Up All SciTops-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p scitops-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/scitops/scitops/issues
