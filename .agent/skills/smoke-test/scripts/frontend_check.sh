...
# When the gateway has authentication enabled (SCITOPS_AUTH_DISABLED != 1),
...
SMOKE_TEST_EMAIL="${SMOKE_TEST_EMAIL:-smoke-test@scitops.dev}"
SMOKE_TEST_PASSWORD="${SMOKE_TEST_PASSWORD:-SmokeTest123!}"
COOKIE_JAR=$(mktemp /tmp/scitops-smoke-cookies.XXXXXX)
