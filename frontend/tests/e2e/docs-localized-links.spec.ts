...
test.describe("Localized documentation links", () => {
  test("keeps English card navigation in the English docs", async ({
    ...
    const card = page.locator("a.nextra-card", { hasText: "Why Quill" });
    await expect(card).toHaveAttribute(
      "href",
      "/en/docs/introduction/why-quill",
    );

    await card.click();
    await expect(page).toHaveURL(/\/en\/docs\/introduction\/why-quill$/);
    await expect(page.locator("main h1")).toContainText("Why Quill");
  });
