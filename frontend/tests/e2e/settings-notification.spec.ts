...
declare global {
  interface Window {
    __quillNotifications?: Array<{ title: string; body?: string }>;
  }
}
  ...
  await page.addInitScript((permission) => {
    ...
    class MockNotification {
      ...
      constructor(title: string, options?: NotificationOptions) {
        window.__quillNotifications?.push({
          title,
          body: options?.body,
        });
      }
...
test.describe("Notification settings", () => {
  test("can request permission and send the first test notification immediately", async ({
    ...
    await expect
      .poll(() => page.evaluate(() => window.__quillNotifications ?? []))
      .toEqual([
        {
          title: "Quill",
