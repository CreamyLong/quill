...
describe("tool message grouping robustness", () => {
  test("identifies ask_clarification tool result by matching tool_call_id even without name", () => {
    const messages = [
      { id: "human-1", type: "human", content: "hello" },
      {
        ...
        content: "",
        tool_calls: [
          { id: "tc-1", name: "ask_clarification", args: { question: "Which?" } },
        ],
      },
      {
        id: "tool-1",
  ...
  test("creates a processing group for orphaned tool messages instead of dropping them", () => {
    const messages = [
      { id: "human-1", type: "human", content: "hello" },
      {
        id: "ai-1",
...
describe("parseClarificationContent", () => {
  test("parses TS ask_clarification JSON envelope", () => {
    const payload = parseClarificationContent(
      JSON.stringify({
        ok: true,
        ...
        clarification: {
          ...
          context: "I need this to proceed.",
  ...
  test("returns null for plain-text clarification content", () => {
    expect(parseClarificationContent("❓ What is your budget?")).toBeNull();
  });

  test("returns null for unrelated JSON", () => {
    expect(parseClarificationContent(JSON.stringify({ foo: "bar" }))).toBeNull();
  });
});

describe("formatClarificationPayload", () => {
  test("formats question with context and options", () => {
    const formatted = formatClarificationPayload({
      question: "What is your budget?",
      clarification_type: "missing_info",
      context: "I need this to proceed.",
      options: ["Low", "Medium", "High"],
    ...
    expect(formatted).toBe(
      "❓ I need this to proceed.\n\nWhat is your budget?\n\n  1. Low\n  2. Medium\n  3. High",
    );
  });

  test("formats question without context or options", () => {
    const formatted = formatClarificationPayload({
      question: "What is your budget?",
    });
    expect(formatted).toBe("❓ What is your budget?");
  });
});
