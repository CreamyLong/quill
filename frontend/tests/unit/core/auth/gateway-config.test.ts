...
describe("getGatewayConfig", () => {
  ...
  beforeEach(() => {
    saved = snapshotEnv();
    setEnv("QUILL_INTERNAL_GATEWAY_BASE_URL", undefined);
    setEnv("QUILL_TRUSTED_ORIGINS", undefined);
  });
  ...
  test("uses env values verbatim when set, regardless of NODE_ENV", async () => {
    setEnv("NODE_ENV", "production");
    setEnv("QUILL_INTERNAL_GATEWAY_BASE_URL", "https://gw.example.com/");
    setEnv(
      "QUILL_TRUSTED_ORIGINS",
