...
import { humanizeVirtualPaths } from "@/core/threads/paths";
...
function MessageContent_({
  ...
  const { workspaceDirectory } = useThread();
  ...
  const contentToDisplay = useMemo(() => {
    const base = (() => {
      ...
      // Strip backend-injected markers (<memory>, <system-reminder>, <current_date>,
      // etc.) from assistant responses — the model can echo its own context
      // verbatim, and that is a privacy leak if left in the chat body.
      return rawContent ? stripInternalMarkers(rawContent) : "";
    })();
    return humanizeVirtualPaths(base, workspaceDirectory);
  }, [rawContent, isHuman, workspaceDirectory]);
