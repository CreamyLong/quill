...
function ToolCall({
  ...
  if (name === "web_search") {
    ...
    const output = result === undefined || result === null ? undefined : typeof result === "string" ? result : typeof result === "object" ? (() => {
      try {
        return JSON.stringify(result, null, 2);
      } catch {
        return String(result as unknown);
      }
    })() : String(result);
    return (
      <ChainOfThoughtStep
        key={id}
        label={resolveLabel(description ?? t.toolCalls.executeCommand)}
        ...
        {output && (
          <details className="mt-1">
            <summary className="cursor-pointer text-xs opacity-60 hover:opacity-100">
              {t.toolCalls.viewOutput}
            </summary>
            <CodeBlock
              className="mx-0 mt-1 border-none px-0"
              showLineNumbers={false}
              language="shell"
              code={output.slice(0, 20000)}
            />
          </details>
        )}
      </ChainOfThoughtStep>
    ...
    if (message.type === "ai") {
      ...
      for (const tool_call of message.tool_calls ?? []) {
        ...
        if (toolCallId) {
          ...
          if (toolCallResult) {
            // Only parse as JSON when the payload is explicitly structured
            // data (object/array). Plain-text tool output (e.g. bash, read_file)
            // can otherwise be mis-parsed into objects and break the string
            // rendering contract downstream.
            const trimmed = toolCallResult.trim();
