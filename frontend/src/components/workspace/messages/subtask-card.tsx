...
export function SubtaskCard({
  ...
  runId?: string;
  isLoading: boolean;
  /** When true, suppress the collapsed-status badge (group summary already shows it). */
  hideStatus?: boolean;
}) {
  const { t } = useI18n();
  ...
  const humanize = useCallback(
    (text: string | null | undefined) =>
      typeof text === "string"
        ? humanizeVirtualPaths(text, workspaceDirectory)
        : "",
    [workspaceDirectory],
  );
  return (
                  ...
                  >
                    ...
                    >
                        ...
                        hasToolCalls(task.latestMessage)
                          ? humanize(explainLastToolCall(task.latestMessage, t))
