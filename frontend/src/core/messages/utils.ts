...
export function getMessageGroups(messages: Message[]): MessageGroup[] {
  ...
  // Map tool_call_id -> tool name for recent AI messages, so we can identify
  // clarification/subagent/present-files tool results even when the backend
  // does not preserve the `name` field on ToolMessage serialization.
  ...
  for (const message of messages) {
    ...
    if (message.type === "ai") {
      // Remember tool_call names for later tool-message classification.
      ...
      } else if (hasToolCalls(message) || (hasReasoning(message) && !hasContent(message))) {
        const lastGroup = groups[groups.length - 1];
        // Accumulate consecutive intermediate AI messages (pure reasoning or
        // tool calls, but not a finalized message that already has answer text)
        // into one processing group.
      ...
      // Not an else-if: a message with reasoning + content (but no tool calls)
      // is rendered as a single assistant bubble here; the processing group
      // above is only created when the message has no displayable content.
    ...
    if (message.type === "tool") {
      const toolName = message.name ?? toolCallIdToName.get(message.tool_call_id ?? "");
      const isClarification = toolName === "ask_clarification";
      if (isClarification) {
        ...
        if (open) {
          ...
          // Orphaned tool message (no preceding AI tool-call group). Rather than
          // dropping it with a console error, create a processing group so the
          // user can still see the tool result and the UI doesn't crash.
          groups.push({
            id: message.id,
            type: "assistant:processing",
            messages: [message],
...
/**
 * Parse a clarification ToolMessage content.
 *
 * The TS `ask_clarification` tool returns a JSON envelope
 * `{ok, message, clarification: {question, context, options, clarification_type}}`
 * when the `ClarificationMiddleware` does not intercept the call. The Python tool
 * returns plain text. This helper normalises both shapes so the UI can render a
 * user-friendly question regardless of which runtime produced the message.
 */
export function parseClarificationContent(
  content: string,
  ...
  try {
    ...
    if (
      ...
      typeof (parsed as Record<string, unknown>).clarification === "object" &&
      (parsed as Record<string, unknown>).clarification !== null
    ) {
      const clarification = (parsed as Record<string, unknown>)
        .clarification as Record<string, unknown>;
      const question = clarification.question;
      if (typeof question !== "string") {
        return null;
      }
      const options: string[] = Array.isArray(clarification.options)
        ? clarification.options.filter((o): o is string => typeof o === "string")
        : [];
      return {
        question,
        clarification_type:
          typeof clarification.clarification_type === "string"
    ...
    // Not the JSON envelope we expected; fall through to raw content rendering.
  }
  return null;
}

/**
 * Format a clarification payload into the same plain-text shape the
 * `ClarificationMiddleware` produces, so the existing Markdown rendering path
 * stays consistent.
 */
export function formatClarificationPayload(payload: ClarificationPayload): string {
  ...
  const icon = typeIcons[payload.clarification_type ?? "missing_info"] ?? "❓";
  const parts: string[] = [];
  if (payload.context) {
    parts.push(`${icon} ${payload.context}`);
    parts.push(`\n${payload.question}`);
  } else {
    parts.push(`${icon} ${payload.question}`);
  }
