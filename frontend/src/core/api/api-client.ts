...
import type { AgentThread, AgentThreadState } from "../threads/types";
...
function createStaticClient(): LangGraphClient {
  ...
  client.threads.get = (async (threadId: string) => {
    return loadStaticDemoThread(threadId) as unknown as AgentThread;
  }) as typeof client.threads.get;

  client.threads.getState = (async (threadId: string) => {
    return staticDemoThreadState(await loadStaticDemoThread(threadId));
  }) as typeof client.threads.getState;

  client.threads.getHistory = (async (threadId: string) => {
    return [staticDemoThreadState(await loadStaticDemoThread(threadId))];
  }) as typeof client.threads.getHistory;

  client.threads.update = (async (threadId: string, _payload?: unknown) => {
    return loadStaticDemoThread(threadId) as unknown as AgentThread;
