...
type SendMessageOptions = {
  ...
  /** Override the workspace directory for this specific run. Takes
   *  precedence over settings.context.workspace_directory so the run always
   *  sees the directory the user just picked, even before the settings
   *  store has re-rendered. */
...
const SUMMARIZATION_MIDDLEWARE_UPDATE_KEYS = new Set([
  ...
  "QuillSummarizationMiddleware.before_model",
]);
...
export function useThreadStream({
  ...
  const thread = useStream<AgentThreadState>({
    ...
    // reconnectOnMount is intentionally false: the SDK's joinStream replays
    // ALL SSE events from event id "-1", which blocks the main thread for the
    // entire duration of a long-running conversation and produces a white
    // screen until the replay finishes. Instead, fetchStateHistory (below)
    // restores the current state instantly, and we join the stream manually
    // only if the run is still active.
    ...
    onCustomEvent(event: unknown) {
      ...
      if (type === "task_started") {
        const e = event as { type: "task_started"; task_id: string; description?: string };
        updateSubtask({ id: e.task_id, status: "in_progress" });
        return;
      }

      if (type === "task_completed") {
        const e = event as { type: "task_completed"; task_id: string; result?: string };
        updateSubtask({ id: e.task_id, status: "completed", result: e.result });
        return;
      }

      if (type === "task_failed") {
        const e = event as { type: "task_failed"; task_id: string; error?: string };
        updateSubtask({ id: e.task_id, status: "failed", error: e.error });
        return;
      }

      if (type === "task_timed_out") {
        const e = event as { type: "task_timed_out"; task_id: string; error?: string };
        updateSubtask({ id: e.task_id, status: "failed", error: e.error });
        return;
      }

      if (type === "task_cancelled") {
        const e = event as { type: "task_cancelled"; task_id: string; error?: string };
        updateSubtask({ id: e.task_id, status: "failed", error: e.error });
        return;
      }
  ...
  // Clear stale reconnect state when switching threads. The SDK stores the
  // active runId in sessionStorage under `lg:stream:{threadId}` for
  // reconnectOnMount. Because we disable that flag (the SDK's joinStream
  // replays ALL events from id "-1", which blocks the main thread for long
  // conversations and produces a white screen), we must clear stale entries
  // to prevent accidental full replays on remount.
  useEffect(() => {
    if (!onStreamThreadId || isMock) return;
    // If this thread has no stored run, nothing to clean up.
    if (!sessionStorage.getItem(`lg:stream:${onStreamThreadId}`)) return;
    // If the SDK already started streaming this thread (onCreated fired), keep it.
    if (startedRef.current) return;
    // No run in flight for this thread — clear any stale entry from a prior session.
    sessionStorage.removeItem(`lg:stream:${onStreamThreadId}`);
                  ...
                  (context.mode === "ultra"
                    ...
                    : context.mode === "pro"
                      ...
                      : context.mode === "thinking"
                        ...
                        : "minimal"),
                thread_id: threadId,
                // Runtime override (Bug 2 fix): the picker value takes
                // precedence over the settings store (which updates async).
