...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. SciTops provides built-in
...
SciTops organizes tools into four categories:
...
```yaml
tools:
  - use: scitops.sandbox.tools:ls_tool
  - use: scitops.sandbox.tools:read_file_tool
  - use: scitops.sandbox.tools:glob_tool
  - use: scitops.sandbox.tools:grep_tool
  - use: scitops.sandbox.tools:write_file_tool
  - use: scitops.sandbox.tools:str_replace_tool
  - use: scitops.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
```yaml
tools:
  - use: scitops.community.ddg_search.tools:web_search_tool
```
...
```yaml
tools:
  - use: scitops.community.tavily.tools:web_search_tool
    api_key: $TAVILY_API_KEY
```
High-quality search with structured results. Requires a [Tavily](https://tavily.com) API key.

Install: `cd backend && uv add 'scitops-harness[tavily]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.brave.tools:web_search_tool
...
```yaml
tools:
  - use: scitops.community.exa.tools:web_search_tool
    api_key: $EXA_API_KEY
```
Semantic search with neural retrieval. Requires an [Exa](https://exa.ai) API key.

Install: `cd backend && uv add 'scitops-harness[exa]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.infoquest.tools:web_search_tool
...
```yaml
tools:
  - use: scitops.community.firecrawl.tools:web_search_tool
    api_key: $FIRECRAWL_API_KEY
```
Firecrawl-powered search and crawl. Requires a [Firecrawl](https://firecrawl.dev) API key.

Install: `cd backend && uv add 'scitops-harness[firecrawl]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.groundroute.tools:web_search_tool
...
Install: `cd backend && uv add 'scitops-harness[groundroute]'`
...
```yaml
tools:
  - use: scitops.community.jina_ai.tools:web_fetch_tool
...
tools:
  - name: web_fetch
    group: web
    use: scitops.community.browserless.tools:web_fetch_tool
...
```yaml
tools:
  - use: scitops.community.exa.tools:web_fetch_tool
    api_key: $EXA_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.infoquest.tools:web_fetch_tool
    api_key: $INFOQUEST_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.firecrawl.tools:web_fetch_tool
    api_key: $FIRECRAWL_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: scitops.community.groundroute.tools:web_fetch_tool
    api_key: $GROUNDROUTE_API_KEY
```
  </Tabs.Tab>
</Tabs>
...
tools:
  - name: web_capture
    group: web
    use: scitops.community.browserless.tools:web_capture_tool
    base_url: http://localhost:3032
    # token: $BROWSERLESS_TOKEN # required for Browserless Cloud; omit for self-hosted (a $VAR that is unset fails startup)
    output_format: png
    full_page: true
...
through SciTops's artifact system. This is useful for visual evidence, UI checks,
...
```yaml
tools:
  - use: scitops.community.image_search.tools:image_search_tool
```
...
```yaml
tools:
  - use: scitops.community.infoquest.tools:image_search_tool
    api_key: $INFOQUEST_API_KEY
```
...
```yaml
tools:
  - use: scitops.community.serper.tools:image_search_tool
...
```yaml
tools:
  - use: scitops.community.brave.tools:image_search_tool
    api_key: $BRAVE_SEARCH_API_KEY
```
