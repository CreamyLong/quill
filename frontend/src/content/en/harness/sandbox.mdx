...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes SciTops capable of real action, not just
  conversation.
</Callout>
...
SciTops supports four sandbox modes. Choose the one that fits your deployment:
...
sandbox:
  ...
  # Optional: container name prefix (default: scitops-sandbox)
  container_prefix: scitops-sandbox
...
```yaml
sandbox:
  use: scitops.community.e2b_sandbox:E2BSandboxProvider
...
Install: `e2b-code-interpreter` is already a core dependency of `scitops-harness`, so no extra install step is needed — just provide your API key and switch the provider in `config.yaml`.
...
<Callout type="info">
  Because e2b is a remote sandbox, SciTops's `/mnt/user-data` virtual prefix is
...
| `.scitops/threads/{thread_id}/user-data/` | `/mnt/user-data/`                            | Read-write |
...
provider runs inside the `scitops-gateway` container. In that mode,
...
services:
  gateway:
    volumes:
      - ${SCITOPS_REPO_ROOT}/.scitops/knowledge:/app/.scitops/knowledge:ro
```
...
If the configured `host_path` is not visible to the gateway process, SciTops
