---
title: Memory
description: Memory is a runtime feature of the SciTops Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets SciTops carry useful information across sessions. The agent
...
Memory is a runtime feature of the SciTops Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.scitops/memory.json)
  ...
  storage_class: scitops.agents.memory.storage.FileMemoryStorage
...
SciTops supports two levels of memory:
...
- Base directory: `backend/.scitops/`
- Global memory: `backend/.scitops/memory.json`
- Per-agent memory: `backend/.scitops/agents/{agent_name}/memory.json`
