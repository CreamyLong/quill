---
title: Customization
description: SciTops's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  SciTops is designed to be adapted. You can extend agent behavior by writing
...
SciTops's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
from scitops.agents.thread_state import ThreadState
...
```python
from scitops.sandbox.sandbox_provider import SandboxProvider
from scitops.sandbox.sandbox import Sandbox
...
```python
from scitops.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `SciTopsClient`.
...
guardrails:
  enabled: true
  provider:
    use: scitops.guardrails.builtin:AllowlistProvider
    config:
