...
description: This page covers day-to-day operational tasks and solutions to common problems when running SciTops App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running SciTops App.
...
When you pull a new version of SciTops, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.scitops/`.
- Look for memory update errors in `logs/gateway.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.scitops/`:
...
In Docker deployments, the bind-mounted host path (`$SCITOPS_ROOT/backend/.scitops/`) is the source of truth — back up the host path.
