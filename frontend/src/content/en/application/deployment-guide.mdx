...
description: "This guide covers all supported deployment methods for SciTops App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for SciTops App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run SciTops. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your scitops repo root
export SCITOPS_ROOT=/path/to/scitops
...
# .env
OPENAI_API_KEY=sk-...
SCITOPS_ROOT=/absolute/path/to/scitops
...
Thread data is stored in `backend/.scitops/threads/`. In Docker deployments, this directory is bind-mounted into the gateway container.

To avoid data loss when containers are recreated:

1. Set `SCITOPS_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=scitops-userdata-pvc
SKILLS_PVC_NAME=scitops-skills-pvc
```
...
SciTops App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
