---
title: Configuration
description: SciTops App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
SciTops App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
models:
  - name: deepseek-v3
    use: scitops.models.patched_deepseek:PatchedChatDeepSeek
...
Install Ollama provider: `cd backend && uv add 'scitops-harness[ollama]'`
...
```yaml
sandbox:
  use: scitops.sandbox.local:LocalSandboxProvider
...
```yaml
sandbox:
  use: scitops.community.aio_sandbox:AioSandboxProvider
  image: enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest
  replicas: 3
  idle_timeout: 600
```

Install: `cd backend && uv add 'scitops-harness[aio-sandbox]'`
...
tools:
  ...
  - use: scitops.community.ddg_search.tools:web_search_tool # default, no key required
  ...
  - use: scitops.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, SciTops uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.scitops/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/scitops
```
