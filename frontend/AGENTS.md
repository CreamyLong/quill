...
This file provides guidance to AI coding agents (Claude Code, Codex, and others) when working with the Quill frontend. It is the source of truth; the sibling `CLAUDE.md` imports it via `@AGENTS.md`.

## Project Overview

Quill Frontend is a Next.js 16 web interface for an AI agent system. It communicates with a LangGraph-based backend to provide thread-based AI conversations with streaming responses, artifacts, and a skills/tools system.
...
- **Trace correlation**: Every run carries an `X-Trace-Id` header. The input-box displays it for debugging, and it propagates to sub-agents and background threads.
- **Permission levels**: Users have an adaptive permission level (0-4) that controls tool approval friction. Accessible via Settings → Permissions.
