...
This file is for coding agents. If the Quill repository is not already cloned and open, clone `https://github.com/scitops/scitops.git` first, then continue from the repository root.

## Goal

Bootstrap a Quill local development workspace on the user's machine with the least risky path available.
...
- The Quill repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Quill.
...
- If the current directory is not the Quill repository root, clone `https://github.com/scitops/scitops.git` if needed, then change into the repository root.
- Confirm the current directory is the Quill repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start Quill
