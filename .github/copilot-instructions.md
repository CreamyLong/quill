# Copilot Onboarding Instructions for Quill
...
Quill is a full-stack "super agent harness".
...
- `backend/packages/harness/quill/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/quill/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/quill/subagents/` - subagent registry/execution
- `backend/packages/harness/quill/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`quill.agents:make_lead_agent`)
