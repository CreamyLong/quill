...
**Goal:** Add an opt-in community `web_capture` tool backed by Browserless `/screenshot` that renders a URL in headless Chrome, saves a PNG/JPEG/WebP screenshot into the current thread outputs directory, and exposes the file as a Quill artifact path.

**Architecture:** Keep the integration inside the existing `quill.community.browserless` package. `BrowserlessClient` owns HTTP request construction and binary response handling; `tools.py` owns Quill runtime/config resolution, output filename generation, file persistence, and artifact-state updates. The feature uses existing thread-data outputs semantics rather than inventing a new artifact channel.
...
- `backend/packages/harness/quill/community/browserless/__init__.py`: export `web_capture_tool`.
...
**Files:**
- Modify: `backend/packages/harness/quill/community/browserless/browserless_client.py`
- Modify: `backend/tests/test_browserless_client.py`
...
**Files:**
- Modify: `backend/packages/harness/quill/community/browserless/tools.py`
- Modify: `backend/packages/harness/quill/community/browserless/__init__.py`
...
cd backend && uv run ruff check packages/harness/quill/community/browserless tests/test_browserless_client.py
cd backend && uv run ruff format --check packages/harness/quill/community/browserless tests/test_browserless_client.py
```
