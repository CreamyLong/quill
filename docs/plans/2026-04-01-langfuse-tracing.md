...
**Goal:** Add optional Langfuse observability support to Quill while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/quill/config/tracing_config.py packages/harness/quill/models/factory.py packages/harness/quill/tracing`
