#!/usr/bin/env bash
# chat.sh — Send a message to Quill and collect the streaming response.
#
# Usage:
...
#   QUILL_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   QUILL_URL          — Unified proxy base URL (default: http://localhost:2026)
#   QUILL_GATEWAY_URL  — Gateway API base URL (default: $QUILL_URL)
#   QUILL_LANGGRAPH_URL — LangGraph API base URL (default: $QUILL_URL/api/langgraph)
#
...
GATEWAY_URL="${QUILL_GATEWAY_URL:-$QUILL_URL}"
LANGGRAPH_URL="${QUILL_LANGGRAPH_URL:-$QUILL_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: Quill is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure Quill is running. Start it with: cd <quill-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from Quill: {data_str}", file=sys.stderr)
            sys.exit(1)
