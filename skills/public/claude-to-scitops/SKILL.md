---
name: claude-to-quill
description: "Interact with Quill AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to Quill for research/analysis, start a Quill conversation thread, check Quill status or health, list available models/skills/agents in Quill, manage Quill memory, upload files to Quill threads, or delegate complex research tasks to Quill. Also use when the user mentions quill, deer flow, or wants to run a deep research task that Quill can handle."
---

# Quill Skill

Communicate with a running Quill instance via its HTTP API. Quill is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Quill exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$QUILL_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$QUILL_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `QUILL_URL`          | `http://localhost:2026`                  | Unified proxy base URL             |
| `QUILL_GATEWAY_URL`  | `${QUILL_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `QUILL_LANGGRAPH_URL`| `${QUILL_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
QUILL_URL="${QUILL_URL:-http://localhost:2026}"
QUILL_GATEWAY_URL="${QUILL_GATEWAY_URL:-$QUILL_URL}"
QUILL_LANGGRAPH_URL="${QUILL_LANGGRAPH_URL:-$QUILL_URL/api/langgraph}"
```
...
Verify Quill is running:

```bash
curl -s "$QUILL_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$QUILL_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$QUILL_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$QUILL_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$QUILL_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$QUILL_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$QUILL_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$QUILL_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$QUILL_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$QUILL_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$QUILL_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$QUILL_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-quill/scripts/chat.sh "Your question here"
```
...
- If health check fails, Quill is not running. Inform the user they need to start it.
