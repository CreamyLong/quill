#!/usr/bin/env python3
"""Create a redacted Quill support bundle for community troubleshooting."""
...
def _candidate_thread_data_dirs(project_root: Path, thread_id: str) -> list[Path]:
    ...
    candidates = [
        project_root / ".scitops" / "threads" / thread_id / "user-data",
        project_root / "backend" / ".scitops" / "threads" / thread_id / "user-data",
    ]
    for base in (project_root / ".scitops" / "users", project_root / "backend" / ".scitops" / "users"):
        if base.exists():
...
def render_ai_issue_draft(triage: dict[str, Any], issue_summary: str, doctor: dict[str, Any] | None) -> str:
    ...
    lines = [
        "# AI issue draft",
        "",
        "Use this when a coding agent or AI assistant files a Quill bug report.",
        ...
        "### Before you start",
        "",
        "- [ ] I searched [existing issues](https://github.com/scitops/scitops/issues?q=is%3Aissue) and this is not a duplicate.",
...
def _default_out_path(project_root: Path) -> Path:
    ...
    return project_root / ".scitops" / "support-bundles" / f"quill-support-bundle-{timestamp}.zip"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project-root", type=Path, default=repo_root, help="Quill project root")
