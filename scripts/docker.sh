...
COMPOSE_CMD="docker compose -p quill-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"quill.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"quill.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  Quill Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting Quill Docker Development"
    echo "=========================================="
    echo ""
    ...
    if [ "$sandbox_mode" = "aio" ]; then
        local docker_socket="${QUILL_DOCKER_SOCKET:-/var/run/docker.sock}"
    ...
    # Set QUILL_ROOT for provisioner if not already set
    if [ -z "$QUILL_ROOT" ]; then
        export QUILL_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting QUILL_ROOT=$QUILL_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting Quill.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
...
stop() {
    # QUILL_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" quill-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting Quill Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "Quill Docker Management Script"
    echo ""
