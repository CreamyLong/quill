#!/usr/bin/env python3
"""Quill Health Check (make doctor).
...
def _load_app_config(config_path: Path) -> object:
    from quill.config.app_config import AppConfig
...
def check_web_tool(config_path: Path, *, tool_name: str, label: str) -> CheckResult:
    ...
    try:
        ...
        free_providers = {
            ...
            "image_search": {"quill.community.image_search.tools": "DuckDuckGo Images (no key needed)"},
        }
        key_providers = {
            "web_search": {
...
def main() -> int:
    ...
    if total_fails == 0 and total_warns == 0:
        ...
        print(f"Run {cyan('make dev')} to start Quill")
    elif total_fails == 0:
