# Contributing to Quill Backend

Thank you for your interest in contributing to Quill! This document provides guidelines and instructions for contributing to the backend codebase.
...
```
backend/
├── packages/harness/quill/  # quill-harness package (import: quill.*)
│   ├── agents/                 # Agent system
...
```python
import pytest
from quill.models.factory import create_chat_model
...
1. Create tool in `packages/harness/quill/tools/builtins/` or `packages/harness/quill/community/`:

```python
# packages/harness/quill/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: quill.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/quill/agents/middlewares/`:

```python
# packages/harness/quill/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/quill/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/quill/config/app_config.py` with new fields
