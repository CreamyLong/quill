...
Used by ``frontend/playwright.record.config.ts``. Env:
  ...
  QUILL_RECORD_OUT               - JSONL path to append captured turns to
...
def _install_capture(out_path: Path) -> None:
    ...
    import quill.models.factory as factory_mod
...
def main() -> int:
    ...
    record_out = os.environ.get("QUILL_RECORD_OUT")
    if not record_out:
        print("ERROR: set QUILL_RECORD_OUT to the JSONL path to append captured turns to", file=sys.stderr)
        return 2
    ...
    # QUILL_HOME can't leak in and shift prompt-affecting paths/skills.
    os.environ["QUILL_HOME"] = str(home)
    os.environ["QUILL_CONFIG_PATH"] = str(cfg)
    os.environ["QUILL_EXTENSIONS_CONFIG_PATH"] = str(prepare_hermetic_extras(home))
