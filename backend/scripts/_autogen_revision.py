...
1. ``alembic.ini``'s default ``sqlalchemy.url`` (``sqlite:///./data/quill.db``)
...
``packages/harness/quill/persistence/migrations/versions/`` -- exactly
...
import quill.persistence.models  # noqa: F401  -- registers ORM models with Base.metadata
from quill.persistence.bootstrap import _escape_url_for_alembic

BACKEND_DIR = Path(__file__).resolve().parents[1]
MIGRATIONS_DIR = BACKEND_DIR / "packages/harness/quill/persistence/migrations"
...
def _build_temp_db_at_head() -> str:
    tmpdir = tempfile.mkdtemp(prefix="quill-autogen-")
