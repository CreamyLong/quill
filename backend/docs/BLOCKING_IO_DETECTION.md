...
This document describes how to use and maintain Quill backend blocking-IO
...
```text
.scitops/blocking-io-findings.json
```
...
focused test when code under `app.*` or `quill.*` performs blocking IO on
...
Keeping rules centralized makes it clear which extra primitives Quill
