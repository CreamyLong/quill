...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Quill 2.0.
...
from quill.agents.lead_agent.agent import make_lead_agent
...
### Agent Module
- **Location**: `packages/harness/quill/agents/lead_agent/agent.py`
...
Quill uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Quill prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Quill's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/quill/backend/packages/harness/quill/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Quill-style prompts**
...
- Custom prompts emphasize the same principles as Quill's main system prompt (clarity, action-oriented, critical rules)
