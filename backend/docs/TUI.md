# Quill Terminal Workbench (TUI)

`quill` is a terminal-native workbench for the Quill harness. It runs
**embedded** over `QuillClient` — no Gateway, frontend, nginx, or Docker
services required — while honoring the same `config.yaml`, checkpointer, skills,
memory, MCP, and sandbox settings as the rest of Quill.

![Quill TUI](../../docs/tui/tui-preview.svg)
...
```bash
uv pip install 'quill-harness[tui]'    # or: pip install textual
```
...
| Command | Behavior |
|---|---|
| `quill` | Launch the TUI when stdin/stdout are TTYs |
| `quill --tui` | Force the TUI (clear diagnostic if `textual` is missing) |
| `quill --cli` | Force headless/classic mode for one invocation |
| `quill chat` | Same TUI conversation surface |
| `quill --continue` | Resume the most recent thread |
| `quill --resume THREAD` | Resume a thread by id |
| `quill --print "question"` | Headless one-shot answer to stdout |
| `quill --json "question"` | Headless newline-delimited `StreamEvent`s |
| `echo "q" \| quill --print` | Read the message from stdin |
| `QUILL_TUI=1 quill` | Force the TUI via environment |

If no TTY is available and no headless flag is given, `quill` prints guidance
instead of hanging.
...
semantics as elsewhere in Quill). `/model` and `/threads` open modal pickers.
...
session.py      builds QuillClient (+ checkpointer) and the persistence writer
...
`QuillClient.stream()` is a **synchronous** generator, so the app runs it on a
...
`quill.persistence.engine.init_engine_from_config`), **not** the Gateway
