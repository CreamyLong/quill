...
The simplest option. Ships with Quill. Block or allow tools by name. No external packages, no passport, no network.
...
guardrails:
  enabled: true
  provider:
    use: quill.guardrails.builtin:AllowlistProvider
    config:
...
aport setup --framework quill
```

This creates:
- `~/.aport/quill/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/quill/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; Quill doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the Quill middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start Quill and ask: "Create a file called test.txt with content hello"
...
class MyGuardrailProvider:
    ...
    def evaluate(self, request):
        from quill.guardrails.provider import GuardrailDecision, GuardrailReason
...
2. Add the config
3. Start Quill and ask: "Use bash to delete test.txt"
...
| Field | Example use |
|---|---|
| `user_id` | Attach the authenticated Quill user to a provider-side policy or audit record |
...
class ContextAwareGuardrailProvider:
    ...
    async def aevaluate(self, request):
        ...
        # (Quill enforces a blocking-IO gate in CI). If your policy
    ...
    def _decide(self, request):
        # 1. Normalize Quill request data into policy context.
        context = {
        ...
        # 3. Convert the policy result back to Quill's decision object.
        return GuardrailDecision(
            allow=result["allow"],
            reasons=[
                GuardrailReason(
...
Quill loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="quill"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="quill" tells you which config dir to use
        ...
```
...
```
packages/harness/quill/guardrails/
...
packages/harness/quill/config/
    guardrails_config.py     # GuardrailsConfig Pydantic model + singleton

packages/harness/quill/agents/middlewares/
