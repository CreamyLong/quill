...
This guide explains how to configure Quill for your environment.
...
- Xiaomi MiMo (`quill.models.patched_mimo:PatchedChatMiMo`)
- Claude Code OAuth (`quill.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`quill.models.openai_codex_provider:CodexChatModel`)
...
- On macOS, Quill does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `quill.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:
...
models:
  - name: mimo-v2.5-pro
    ...
    use: quill.models.patched_mimo:PatchedChatMiMo
    model: mimo-v2.5-pro
...
tools:
  - name: web_capture
    group: web
    use: quill.community.browserless.tools:web_capture_tool
...
Quill supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: quill.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: quill.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```
...
```yaml
sandbox:
   use: quill.community.aio_sandbox:AioSandboxProvider
   provisioner_url: http://provisioner:8002
```

When using Docker development (`make docker-start`), Quill starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
```yaml
sandbox:
   use: quill.community.e2b_sandbox:E2BSandboxProvider
...
`e2b-code-interpreter` is bundled as a core dependency of `quill-harness`,
...
- Each Quill thread is bound to its e2b sandbox via metadata
...
`allow_host_bash` is intentionally `false` by default. Quill's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.

When `LocalSandboxProvider` runs under `make up`, it runs inside the `quill-gateway` container. In that mode, `sandbox.mounts[].host_path` is resolved from the gateway container's filesystem, not from your Docker host. If you need a local-sandbox custom mount in production Docker, bind the host directory into the gateway service first, then use the in-container path in `config.yaml`:
...
services:
  gateway:
    volumes:
      - ${QUILL_REPO_ROOT}/.scitops/knowledge:/app/.scitops/knowledge:ro
```
...
sandbox:
  use: quill.sandbox.local:LocalSandboxProvider
  mounts:
    - host_path: /app/.scitops/knowledge
      container_path: /mnt/knowledge
      read_only: true
```

If the configured `host_path` is not visible to the gateway process, Quill logs an error and ignores that mount.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: quill-sandbox
...
When you configure `sandbox.mounts`, Quill exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

For bare-metal Docker sandbox runs that use localhost, Quill binds the sandbox HTTP port to `127.0.0.1` by default so it is not exposed on every host interface. Docker-outside-of-Docker deployments that connect through `host.docker.internal` keep the broad legacy bind for compatibility. Set `QUILL_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address.

### Building a Custom AIO Sandbox Image

`AioSandboxProvider` talks to the sandbox container through the `agent-sandbox` SDK. The Dockerfile for the default `enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest` image is not part of this repository; Quill treats that image as an upstream AIO sandbox runtime.
...
USER root
# Example user dependency; not required by Quill itself.
RUN apt-get update \
...
If you rebuild the runtime from scratch instead of extending the published image, it must expose the same HTTP API used by `agent-sandbox`. Quill currently depends on:
...
- `/mnt/user-data` must remain writable because Quill mounts thread workspace, uploads, and outputs there.
- `home_dir` comes from the sandbox context endpoint; do not assume Quill hardcodes it.
- Shell command handling must remain compatible with serialized `exec_command` calls. Quill serializes shell access on the host side to avoid corrupting the sandbox's persistent shell session.
...
**How Skills Work**:
- Skills are stored in `quill/skills/{public,custom}/`
...
2. Restart the Quill service to apply changes

## Environment Variables

Quill supports environment variable substitution using the `$` prefix:
...
- `QUILL_PROJECT_ROOT` - Project root for relative runtime paths
- `QUILL_CONFIG_PATH` - Custom config file path
- `QUILL_EXTENSIONS_CONFIG_PATH` - Custom extensions config file path
- `QUILL_HOME` - Runtime state directory (defaults to `.scitops` under the project root)
- `QUILL_SKILLS_PATH` - Skills directory when `skills.path` is omitted
...
The configuration file should be placed in the **project root directory** (`quill/config.yaml`). Set `QUILL_PROJECT_ROOT` when the process may start from another working directory, or set `QUILL_CONFIG_PATH` to point at a specific file.

## Configuration Priority

Quill searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `QUILL_CONFIG_PATH` environment variable
3. `config.yaml` under `QUILL_PROJECT_ROOT`, or under the current working directory when `QUILL_PROJECT_ROOT` is unset
4. Legacy backend/repository-root locations for monorepo compatibility

## Security Notes
### Sandbox Isolation and the Docker Socket (DooD)

Quill executes agent-generated shell/code through a configurable sandbox
...
|------|---------------|--------------------|-----------|
| `local` (default) | `quill.sandbox.local:LocalSandboxProvider` | Not mounted | Commands run **inside the gateway container** on its filesystem. Not a strong boundary — `allow_host_bash` is `false` by default and should stay off for untrusted workloads. |
| `aio` (pure DooD) | `quill.community.aio_sandbox:AioSandboxProvider` (no `provisioner_url`) | **Mounted** (opt-in overlay) | Sandbox containers are started via the host Docker daemon. |
...
matters for Quill because the gateway executes model-generated commands, so a
...
Quill can reuse your Claude Code / Codex CLI subscription login as a model
...
adapters authenticate independently of Quill via their own documented env —
...
1. **Place `config.yaml` in project root** - Set `QUILL_PROJECT_ROOT` if the runtime starts elsewhere
...
- Ensure `config.yaml` exists in the **project root** directory (`quill/config.yaml`)
- If the runtime starts outside the project root, set `QUILL_PROJECT_ROOT`
- Alternatively, set `QUILL_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `quill/skills/` directory exists
- Verify skills have valid `SKILL.md` files
- Check `skills.path` or `QUILL_SKILLS_PATH` if using a custom path
