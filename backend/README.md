...
Quill is a LangGraph-based AI super agent with sandbox execution, persistent memory, and extensible tool integration. The backend enables AI agents to execute code, browse the web, manage files, delegate tasks to subagents, and retain context across conversations - all in isolated, per-thread environments.
...
- **Skills path**: `/mnt/skills` → `quill/skills/` directory
...
| `DELETE /api/threads/{id}` | Delete Quill-managed local thread data after LangGraph thread deletion; unexpected failures are logged server-side and return a generic 500 detail |
...
For Feishu card updates, Quill stores the running card's `message_id` per inbound message and patches that same card until the run finishes, preserving the existing `OK` / `DONE` reaction flow.
...
```bash
uv pip install 'quill-harness[tui]'   # optional 'textual' dependency
quill                                 # launch the TUI
quill --print "summarize this repo"   # headless one-shot
```
...
```
backend/
├── packages/harness/           # quill-harness package (import: quill.*)
│   └── quill/
│       ├── agents/             # Agent system
...
│       ├── tui/                # Terminal UI (`quill` console script)
...
- If a provider module is missing, Quill now returns an actionable error with install guidance (for example `uv add langchain-google-genai`).
...
- `QUILL_CONFIG_PATH` - Override config.yaml location
- `QUILL_EXTENSIONS_CONFIG_PATH` - Override extensions_config.json location
- Model API keys: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, etc.
- Tool API keys: `TAVILY_API_KEY`, `GITHUB_TOKEN`, etc.

### LangSmith Tracing

Quill has built-in [LangSmith](https://smith.langchain.com) integration for observability. When enabled, all LLM calls, agent runs, tool executions, and middleware processing are traced and visible in the LangSmith dashboard.
...
Quill also supports [Langfuse](https://langfuse.com) observability for LangChain-compatible runs.
...
If both LangSmith and Langfuse are enabled, Quill initializes and attaches both callbacks so the same run data is reported to both systems.

If a provider is explicitly enabled but required credentials are missing, or the provider callback cannot be initialized, Quill raises an error when tracing is initialized during model creation instead of silently disabling tracing.
...
Quill's application tables (`runs`, `threads_meta`, `feedback`, `users`,
...
`packages/harness/quill/persistence/migrations/versions/`:
...
does not need a pre-existing `./data/quill.db`. Review the generated file
...
`.scitops/blocking-io-findings.json` at the repository root (regardless of
